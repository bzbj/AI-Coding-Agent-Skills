...
- File paths ending with `.pdf`
- Keywords: "extract text from pdf", "merge pdf", "split pdf", "fill pdf form", "create pdf", "convert to pdf", "pdf to"
- Chinese keywords: "招股说明书", "年报", "财报" (财务报告相关的 PDF)

**DO NOT** attempt to read PDFs directly with Read tool or other methods. Always use this skill first.

---

## Machine Notes

- On this machine, the `process` route works in WSL after installing `pikepdf` and LibreOffice.
- The HTML route works, but in this Codex environment it may require escalated execution because Chromium launch can be blocked by the current sandbox.
- Treat `pdf.sh html ...` as a command that may need approval.
- `tectonic` is not currently installed, so LaTeX route availability is limited.
