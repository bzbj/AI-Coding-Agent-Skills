...
- File paths ending with `.xlsx`, `.xls`, or `.csv`
- Keywords: "excel", "spreadsheet", "create xlsx", "pivot table", "数据透视表", "formula", "chart in excel"
- Chinese keywords: "财务分析", "报表", "数据表", "透视表"

**DO NOT** attempt to read Excel files directly with Read tool if data analysis or formulas are involved. Always use this skill first.

---

## Machine Notes

- On this machine, `inspect`, `reference-check`, and `validate` are usable in WSL.
- The bundled `recheck` command is currently not reliable in this installation. It fails during LibreOffice macro setup/configuration even after LibreOffice installation.
- Do not block the whole workbook workflow on `recheck` here unless this installation is repaired or replaced.
