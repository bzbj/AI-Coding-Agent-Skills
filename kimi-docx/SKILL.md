...
When user mentions **ANY** of the following, you **MUST** use this skill:
- File paths ending with `.docx` or `.doc`
- Keywords: "create word", "generate docx", "edit word", "modify docx", "track changes", "add comment", "word document"
- Document features: "cover page", "table of contents", "页眉", "页脚", "批注", "修订"

**DO NOT** attempt to read Word documents directly with Read tool if formatting matters. Always use this skill first.

---

## Machine Notes

- On this machine, `./scripts/docx` has been patched to default to `/tmp/kimi-output/docx` instead of `/mnt/okcomputer/output`.
- `docx init` and `docx build` are usable in WSL with the patched script.
- The bundled OpenXML validator package is incomplete in this installation; Python validation still runs, but binary validator checks may be skipped with a warning.
